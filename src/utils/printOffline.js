...
//   - flushQueuedPrintJobs(): retry all queued print jobs when back online
...
async function discoverPrintAgentUrls() {
  ...
  if (configuredUrl && configuredUrl !== 'http://127.0.0.1:3101') {
    add(configuredUrl);
  }
...
export async function testOfflinePrintPipeline() {
  ...
  try {
    const flush = await flushQueuedPrintJobs();
