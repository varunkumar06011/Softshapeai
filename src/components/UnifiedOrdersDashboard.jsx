  ...
  return (
    <div className="space-y-3 rounded-[10px] border border-[#FFCDD2] bg-white p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 text-sm">
        <select className="rounded border border-[#FFCDD2] p-1 w-full" onChange={(e) => setFilter((f) => ({ ...f, platform: e.target.value }))}><option>All</option><option>Zomato</option><option>Swiggy</option><option>Direct</option></select>
        <select className="rounded border border-[#FFCDD2] p-1 w-full" onChange={(e) => setFilter((f) => ({ ...f, branch: e.target.value }))}><option>All</option><option>Main Hall</option><option>Express Outlet</option></select>
        <select className="rounded border border-[#FFCDD2] p-1 w-full" onChange={(e) => setFilter((f) => ({ ...f, status: e.target.value }))}><option>All</option><option>Preparing</option><option>Ready</option><option>Dispatched</option><option>Served</option></select>
        <select className="rounded border border-[#FFCDD2] p-1 w-full" onChange={(e) => setFilter((f) => ({ ...f, time: e.target.value }))}><option value="All">All</option><option value="15m">Last 15m</option><option value="1h">Last 1h</option></select>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
        {visible.slice(0, 9).map((order) => (
          <div key={order.id} className="rounded border border-[#FFCDD2] p-2 text-sm hover:border-[#E53935] transition-colors">
            <div className="flex items-center justify-between">
              <span className="font-semibold">{order.id}</span>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${PLATFORM_META[order.platform]?.badgeClass ?? "bg-slate-100 text-slate-700"}`}>{order.platform}</span>
            </div>
            <p className="font-medium mt-1">{order.customer}</p>
            <p className="text-xs text-[#6B6B6B]">{order.dish}</p>
            <p className="font-bold text-[#E53935] mt-1">₹{order.amount}</p>
            <div className="mt-2 flex items-center justify-between border-t border-[#FFEBEE] pt-1">
              <span className="text-[10px] text-[#6B6B6B]">{order.branch}</span>
