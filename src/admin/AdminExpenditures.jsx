...
export default function AdminExpenditures() {
  ...
  const handleDelete = async (v) => {
    const isVendorPayment = v.entryType === 'LIABILITY_PAYMENT' && v.linkedVendorId;
    const confirmMsg = isVendorPayment
      ? `Delete this vendor payment of ₹${Number(v.amount).toLocaleString()} to ${v.paidToName}?\n\nThe vendor's outstanding balance will be restored and the daily balance sheet will be recalculated.`
      : `Delete this expenditure of ₹${Number(v.amount).toLocaleString()} to ${v.paidToName}?\n\nThis will also remove it from the daily balance sheet and recalculate totals.`;
    if (!window.confirm(confirmMsg)) return;
    setActionLoading((prev) => ({ ...prev, [v.id]: true }));
    setActionError('');
    try {
      await apiFetch(`/api/expenditures/${v.id}`, { method: 'DELETE' });
      loadExpenditures();
    } catch (err) {
      console.error('[AdminExpenditures] Delete failed:', err);
      setActionError(err.message || 'Delete failed');
    } finally {
      setActionLoading((prev) => ({ ...prev, [v.id]: false }));
    }
  };
  ...
  return (
    <div className="space-y-4" data-tour="admin-expenditures">
      {actionError && (
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            data-tour="admin-expenditures-add"
      ...
      {/* Filters */}
      <div className="bg-white rounded-xl border border-gray-200 p-3 flex items-center gap-3 flex-wrap" data-tour="admin-expenditures-categories">
                  ...
                  <tr key={v.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {v.isAutoGenerated && (
                          <span className="text-[9px] font-bold text-gray-400 px-1" title="Auto-generated from Daily Purchase">
                            AUTO
                          </span>
                        )}
                        <button
                        ...
                        {!v.isAutoGenerated && v.status === 'UNVERIFIED' && (
                          <button
                        ...
                        {!v.isAutoGenerated && v.status !== 'VOIDED' && (
                          <button
