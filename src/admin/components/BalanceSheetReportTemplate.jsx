...
function TableRow({ icon: Icon, iconColor, label, amount, muted }) {
  return (
    <div className="flex items-center justify-between py-2.5" style={{ borderBottom: '1px solid #F3F4F6' }}>
      ...
      <span className="text-sm font-bold" style={{ color: '#1E293B' }}>{inr(amount).replace('₹', '')}</span>
    </div>
  );
}
...
function TotalRow({ label, amount, tint = '#E63946' }) {
  return (
    <div className="flex items-center justify-between pt-2.5 mt-1" style={{ borderTop: '2px solid #E2E8F0' }}>
      ...
      <span className="text-sm font-black" style={{ color: tint }}>{inr(amount)}</span>
    </div>
  );
}
    ...
    >
      ...
      <div className="mt-6 flex items-start justify-between">
        ...
        <div className="text-right text-[11px] leading-5" style={{ color: '#6B7280' }}>
          <div><span className="font-bold" style={{ color: '#9CA3AF' }}>REPORT ID: </span>{data.reportId}</div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-5">
        ...
        <div className="rounded-xl border p-4" style={{ borderColor: '#E5E7EB' }}>
          ...
          <TotalRow label="Total Venue Sales" amount={data.venueSales.reduce((s, r) => s + r.amount, 0)} />
        </div>
      ...
      <div className="mt-6 flex items-center justify-between rounded-xl p-5" style={{ background: '#0F172A' }}>
        <div>
          ...
          <div className="text-[10px]" style={{ color: '#9CA3AF' }}>{data.amountInWords}</div>
        </div>
        <div className="rounded-lg border px-4 py-2 text-center" style={{ borderColor: 'rgba(74,222,128,0.5)', borderStyle: 'dashed' }}>
          <div className="text-sm font-bold italic" style={{ color: '#4ADE80' }}>"Thank you!"</div>
          <div className="text-[10px]" style={{ color: '#FBBF24' }}>★★★★★</div>
          <div className="text-[10px]" style={{ color: '#9CA3AF' }}>Keep up the great work.</div>
        </div>
      ...
      <div className="mt-3 text-center text-[10px]" style={{ color: '#9CA3AF' }}>
        ...
        <span className="font-bold" style={{ color: '#475569' }}>Softshape AI</span> — Software that shapes your business from{' '}
        <span className="font-bold" style={{ color: '#E63946' }}>Day 1</span>.
      </div>
    </div>
  );
}
